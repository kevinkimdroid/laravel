...
@section('title', 'Record QBASH Pool League Game')
        ...
        <div class="d-flex justify-content-between align-items-center flex-wrap">
            <div>
                <h3 class="mb-1 fw-bold text-primary">
                    <i class="bi bi-plus-circle me-2"></i>Record QBASH Pool League Game
                </h3>
            </div>
...
<div class="card border-0 shadow-sm">
    <div class="card-body p-4">
        <form method="POST" action="{{ route('qpl-games.store') }}">
            @csrf
            ...
            <div class="row">
                <div class="col-md-6 mb-4">
                    ...
                    <input type="date" 
                           name="game_date" 
                           class="form-control" 
                           value="{{ old('game_date', date('Y-m-d')) }}" 
                           required>
                </div>
                    ...
                    <select name="home_team" class="form-select" required>
                        ...
                        @foreach($members as $member)
                            <option value="{{ $member->name }}"
                                {{ old('home_team') === $member->name ? 'selected' : '' }}>
                ...
                <div class="col-md-6 mb-4">
                    ...
                    <select name="away_team" class="form-select" required>
                        ...
                        @foreach($members as $member)
                            <option value="{{ $member->name }}"
                                {{ old('away_team') === $member->name ? 'selected' : '' }}>
                                {{ $member->member_no }} - {{ $member->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-12 mb-4">
                    ...
                    <textarea name="notes" 
                              class="form-control" 
                              rows="3"
                              placeholder="Additional notes about the game (optional)">{{ old('notes') }}</textarea>
                </div>
            </div>

            <div class="d-flex gap-3 mt-4">
                <button type="submit" class="btn btn-primary btn-lg">
                    <i class="bi bi-check-circle me-2"></i>Add Game
                </button>
